<script lang="ts">
  ...
  let selectedSection: ProductGroup;
  ...
  $: {
    if (selectedSection) {
      selectedSectionId = selectedSection.product_group_id;
    }
    filteredSections = sections.filter(
      ({ product_group_id }) => selectedSectionId !== product_group_id,
    );
    dispatch("change", selectedSection);
  }
  ...
  const handleSearch = async (offset: number) => {
    ...
    try {
      ...
      toast.push("타임딜 섹션 검색에 오류가 발생했습니다.");
    } finally {
      isLoading = false;
    }
  };
  ...
  <Column>
    ...
    <StructuredList condensed flush>
      ...
      <StructuredListBody>
        {#if !selectedSection}
          <StructuredListRow>
            <StructuredListCell>선택된 타임딜 섹션 없음</StructuredListCell>
          </StructuredListRow>
        {:else}
          <StructuredListRow>
            <StructuredListCell>
              {selectedSection.short_title}
            </StructuredListCell>
            ...
            <StructuredListCell>
              {#if selectedSection.products.length > 0}
                <span>
                  {selectedSection.products[0].product.alloff_name}
                  {#if selectedSection.products.length > 1}
                    외 {selectedSection.products.length - 1}개의 상품
                  {/if}
                </span>
              {:else}
                No products
            ...
            <StructuredListCell>
              <Row padding>
                <Button
                  ...
                  kind="ghost"
                  size="small"
                  on:click={handleDetailOpen(selectedSection.product_group_id)}
                />
              </Row>
            </StructuredListCell>
          </StructuredListRow>
...
<Row padding>
  <Column>
    <div class="bx--label">타임딜 섹션 검색</div>
    <Search bind:value={searchQuery} placeholder="타이틀/짧은 타이틀 검색" />
  </Column>
</Row>
<div class="row-right-wrapper">
  <Button on:click={handleSectionSearch}>타임딜 섹션 검색</Button>
</div>
